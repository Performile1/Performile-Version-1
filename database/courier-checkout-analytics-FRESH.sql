-- =====================================================
-- COURIER CHECKOUT POSITION ANALYTICS - FRESH VERSION
-- =====================================================
...
CREATE OR REPLACE FUNCTION aggregate_courier_position_history(target_date DATE DEFAULT (CURRENT_DATE - INTERVAL '1 day')::DATE)
RETURNS INTEGER AS $$
DECLARE
...
BEGIN
  ...
  IF test_courier_id IS NOT NULL AND test_merchant_id IS NOT NULL THEN
    ...
    -- Aggregate the data (explicit DATE cast to avoid type mismatch)
    PERFORM aggregate_courier_position_history((CURRENT_DATE - 1)::DATE);
...
SELECT 
  ...
  ROUND(AVG(ccp.items_count), 1) as avg_items_per_order,
  ROUND(AVG(ccp.package_weight_kg), 2) as avg_package_weight
FROM couriers c
