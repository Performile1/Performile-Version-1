...
RETURNS TABLE (
  ...
  total_claims BIGINT,
  open_claims BIGINT,
  in_review_claims BIGINT,
  approved_claims BIGINT,
  declined_claims BIGINT,
...
BEGIN
  ...
  SELECT 
    ...
    COUNT(*) FILTER (WHERE c.status = 'open')::BIGINT as open_claims,
    COUNT(*) FILTER (WHERE c.status = 'in_review')::BIGINT as in_review_claims,
    COUNT(*) FILTER (WHERE c.status = 'approved')::BIGINT as approved_claims,
    COUNT(*) FILTER (WHERE c.status = 'declined')::BIGINT as declined_claims,
    COUNT(*) FILTER (WHERE c.status = 'closed')::BIGINT as closed_claims,
    COALESCE(SUM(c.claim_amount), 0)::NUMERIC as total_claimed_amount,
    COALESCE(SUM(c.approved_amount), 0)::NUMERIC as total_approved_amount,
    COALESCE(
      AVG(
        CASE 
          WHEN c.resolved_at IS NOT NULL AND c.created_at IS NOT NULL
...
BEGIN
  ...
  SELECT 
    COUNT(*)::BIGINT as total_claims,
    COUNT(*) FILTER (WHERE c.status IN ('open', 'in_review'))::BIGINT as open_claims,
    COUNT(*) FILTER (WHERE c.status IN ('approved', 'declined', 'closed'))::BIGINT as resolved_claims,
    COALESCE(SUM(c.claim_amount), 0)::NUMERIC as total_claimed_amount,
    COALESCE(SUM(c.approved_amount), 0)::NUMERIC as total_approved_amount,
    COALESCE(
      AVG(
    ...
    CASE 
      WHEN COUNT(*) FILTER (WHERE c.status IN ('approved', 'declined', 'closed')) > 0
      THEN (COUNT(*) FILTER (WHERE c.status = 'approved')::NUMERIC / 
            COUNT(*) FILTER (WHERE c.status IN ('approved', 'declined', 'closed'))::NUMERIC * 100)
      ELSE 0
...
-- 
-- Column Names (ACTUAL):
-- - status (NOT claim_status)
-- - resolved_at (NOT resolution_date)
-- - claim_amount (correct)
-- - approved_amount (correct)
--
-- Status Values:
-- - 'open', 'in_review', 'approved', 'declined', 'closed'
-- 
